		...
		public class ImportTask : Importer.ImportTask<ImportResult[]> {
			private class MeshData {
				...
				public Mesh ToMesh(RenderingMode mode) {
					...
					if ((tangents.Count == 0) && (mode == RenderingMode.TRIANGLES || mode == RenderingMode.TRIANGLE_FAN || mode == RenderingMode.TRIANGLE_STRIP)) mesh.RecalculateNormals();
					else mesh.normals = normals.ToArray();

					if ((tangents.Count == 0) && (mode == RenderingMode.TRIANGLES || mode == RenderingMode.TRIANGLE_FAN || mode == RenderingMode.TRIANGLE_STRIP)) mesh.RecalculateTangents();
